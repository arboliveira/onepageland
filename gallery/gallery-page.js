...
export class GalleryPageElement extends CustomaryElement {
    static customary = {
        ...
        hooks: {
            ...
            derive: {
                'grid_mode_class': (el) => el.miniature ? 'miniature' : 'hi-def',
            },
            events: [
                {
                    ...
                    listener: (el, event) => {
                        ...
                        const land = event.composedPath().find(n => n.tagName === 'ONE-PAGE-LAND');
                        ...
                        if (!entry || entry.title === title)
                            return;
                        gallery.previews = gallery.previews.map((p, i) => i === idx ? { ...p, title } : p);
                    },
                },
                {
    ...
    rescale() {
        ...
        */
        ...
        this.style.setProperty('--preview-max-width', `${Math.max(room * aspect, MIN_PREVIEW_WIDTH)}px`);
