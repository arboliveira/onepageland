...
describe(suite.title, async function (){
    ...
    describe('happy day', async function () {
        ...
        it('feeds each preview its own params', async function () {
            ...
            const land = CT.querySelectorAll('one-page-land', gallery)[4] as HTMLElement;
        ...
        it('links each preview to its full page, from the params button', async function () {
            const button = CT.querySelector<HTMLAnchorElement>('.preview .params-btn', gallery);
    ...
    describe('the canvas each preview is painted on', async function () {
        ...
        it('shrinks the render with `zoom`, never with a transform', async function () {
            this.retries(64);
            gallery = CT.querySelector('gallery-page', window);
            const canvas = CT.querySelector<HTMLElement>('.canvas', gallery);
        ...
        it('still measures its viewport units against the real viewport', async function () {
            this.retries(64);
            const screen = CT.querySelector<HTMLElement>('.viewport', gallery);
            const canvas = CT.querySelector<HTMLElement>('.canvas', gallery);
            /* `100vw` by `100vh`, shrunk: the canvas fills its little screen exactly */
            chai.assert.closeTo(canvas.getBoundingClientRect().width,
                screen.getBoundingClientRect().width, 2);
            chai.assert.closeTo(canvas.getBoundingClientRect().height,
                screen.getBoundingClientRect().height, 2);
        });
    });
    ...
    describe('the View Mode button', async function () {
        ...
        it('keeps the whole first preview on screen, green button and all', async function () {
            this.retries(64);
            const preview = CT.querySelector<HTMLElement>('.preview', gallery);
        ...
        it('interact', async function () {
            CT.querySelector<HTMLElement>('.view-mode-btn', gallery).click();
        });
