    ...
    static customary = {
        ...
        hooks: {
            ...
            derive: {
                ...
                'param_size_class': (el) => parseValues(el).size_valid ? 'opt-param active' : 'opt-param',
                'param_fgcolor_text': (el) => {
                ...
                'param_fgcolor_class': (el) => parseValues(el).fgcolor_valid ? 'opt-param active' : 'opt-param',
                'param_class_text': (el) => {
                    const { n, classes, class_valid } = parseValues(el);
                    return class_valid ? `&class_${n}=${encodeURIComponent(classes)}` : `&class_${el.line_number || ''}=`;
                },
                'param_class_class': (el) => parseValues(el).class_valid ? 'opt-param active' : 'opt-param',
            },
            changes: {
                'line_number': (el) => emit_change(el),
                'line_size': (el) => emit_change(el),
                'line_fgcolor': (el) => emit_change(el),
                'line_class': (el) => emit_change(el),
            },
            events: [
                {
...
//# sourceMappingURL=one-page-line-options.js.map