 ...
 */
function assert_applied(window, document_before) {
    assert_captured(document_before);
    chai.assert.notStrictEqual(window.document, document_before, 'expected an apply to navigate this window, replacing its document');
    /*
     ...
     */
    chai.assert.include(window.location.search, THE_PARAM, 'the apply navigated, but dropped the params on the way — a native form GET, not ours');
}
function assert_not_applied(window, document_before) {
    assert_captured(document_before);
    chai.assert.strictEqual(window.document, document_before, 'expected no navigation: this should still be the document we started with');
}
/*
 ...
 */
function assert_captured(document_before) {
    chai.assert.isOk(document_before, 'no starting document was captured — run the whole suite, not a --fgrep subset');
}
/*
...
describe(suite_enter.title, async function () {
    ...
    describe('the markup the Enter paths rest on', async function () {
        ...
        it('keeps the hidden submit button that makes Enter submit at all', async function () {
             ...
             */
            chai.assert.isOk(CT.querySelector('#options-form button[type="submit"][hidden]', element), 'the hidden submit button is load-bearing for Enter-to-submit');
        });
        ...
        it('is the inline branch, so an apply lands here and not on the runner', async function () {
            this.retries(128);
            const element = CT.querySelector('one-page-options', window);
            chai.assert.isNull(window.opener, 'the fixture disowns its opener; without that, an apply navigates the Mocha runner');
            /*
             ...
             */
            chai.assert.isNotOk(element.getAttribute('is_in_window'), 'the fixture is the inline case: no `is_in_window` attribute at all');
            /*
            ...
            chai.assert.deepEqual(buttons, ['move-to-window-btn'], 'inline: the screen offers to move out, and nothing offers to move back in');
        });
    });
...
//# sourceMappingURL=one-page-options-test.js.map